...
class LocalSearchMixedContext(LocalContextBuilder):
    ...
    def build_context(
        ...
        **kwargs: dict[str, Any],
        ...
        selected_entities_start_time = time.time()
        ...
        selected_entities_end_time = time.time()
        selected_entities_time = selected_entities_end_time - selected_entities_start_time
        log.info("selected_entities_time: %f", selected_entities_time)
        ...
        log.info("local_prop: %f", local_prop)
        local_tokens = max(int(max_tokens * local_prop), 0)
        log.info("local_tokens: %d", local_tokens)

        build_local_context_start_time = time.time()
        ...
        build_local_context_end_time = time.time()
        build_local_context_time = build_local_context_end_time - build_local_context_start_time
        log.info("build_local_context_time: %f", build_local_context_time)
        ...
        log.info("text_unit_tokens: %d", text_unit_tokens)
        log.info("text_unit_prop: %f", text_unit_prop)

        build_text_unit_context_start_time = time.time()
        text_unit_context, text_unit_context_data = self._build_text_unit_context(
            selected_entities=selected_entities,
            max_tokens=text_unit_tokens,
            return_candidate_context=return_candidate_context,
        )
        build_text_unit_context_end_time = time.time()
        build_text_unit_context_time = build_text_unit_context_end_time - build_text_unit_context_start_time
        log.info("build_text_unit_context_time: %f", build_text_unit_context_time)
